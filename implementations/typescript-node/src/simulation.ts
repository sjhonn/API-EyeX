export const MACHADO_MODEL = 'machado-2009' as const;
export type SimulationType = 'protanopia' | 'deuteranopia' | 'tritanopia';
export type Matrix3 = [[number, number, number], [number, number, number], [number, number, number]];

const protan: Matrix3[] = [
...
export function isSimulationType(value: unknown): value is SimulationType {
  return value === 'protanopia' || value === 'deuteranopia' || value === 'tritanopia';
}

export function normalizeSimulationHex(value: unknown): string | null {
  return typeof value === 'string' && /^#[0-9A-Fa-f]{6}$/.test(value) ? value.toUpperCase() : null;
}

export function normalizeSimulationSeverity(value: unknown, provided: boolean): number | null {
  if (!provided) return 1;
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1) return null;
  return value;
}

function series(type: SimulationType): Matrix3[] {
  if (type === 'protanopia') return protan;
  if (type === 'deuteranopia') return deutan;
  return tritan;
}

export function machadoMatrix(type: SimulationType, severity: number): Matrix3 {
  const matrices = series(type);
  const scaled = severity * 10;
  const nearest = Math.round(scaled);
  if (Math.abs(scaled - nearest) < 1e-12) return matrices[Math.max(0, Math.min(10, nearest))];
  const lo = Math.floor(scaled);
  const hi = Math.min(10, lo + 1);
  const t = scaled - lo;
  return [0, 1, 2].map((row) => [0, 1, 2].map((col) => matrices[lo][row][col] * (1 - t) + matrices[hi][row][col] * t)) as Matrix3;
}
...
export function simulateMachadoHex(hex: string, type: SimulationType, severity: number): string {
  const channels = [1, 3, 5].map((start) => Number.parseInt(hex.slice(start, start + 2), 16) / 255);
  const linear = channels.map(srgbToLinear);
  const matrix = machadoMatrix(type, severity);
  const out = matrix.map((row) => linearToSrgb(row[0] * linear[0] + row[1] * linear[1] + row[2] * linear[2]));
  const one = (v: number) => Math.max(0, Math.min(255, Math.round(v * 255))).toString(16).padStart(2, '0').toUpperCase();
  return `#${one(out[0])}${one(out[1])}${one(out[2])}`;
}
