...
const MachadoModel = "machado-2009"

var ErrInvalidSeverity = errors.New("severity must be between 0 and 1")

// Machado, Oliveira & Fernandes (2009) pre-computed matrices for anomalous
// trichromacy. The paper provides matrices in 0.1 severity steps. Intermediate
// severities are linearly interpolated between adjacent matrices, as suggested
// by the authors. Matrices are applied in linear-light sRGB.
var protanMatrices = [11][3][3]float64{
...
var deutanMatrices = [11][3][3]float64{
	{{1.000000, 0.000000, 0.000000}, {0.000000, 1.000000, 0.000000}, {0.000000, 0.000000, 1.000000}},
...
var tritanMatrices = [11][3][3]float64{
	{{1.000000, 0.000000, 0.000000}, {0.000000, 1.000000, 0.000000}, {0.000000, 0.000000, 1.000000}},
...
func IsMachadoDeficiency(d Deficiency) bool {
	switch d {
	case Protanopia, Protanomaly, Deuteranopia, Deuteranomaly, Tritanopia, Tritanomaly:
		return true
	default:
		return false
	}
...
func SimulateHexSeverity(hex string, deficiency Deficiency, severity float64) (string, error) {
	rgb, err := ParseHex(hex)
	if err != nil {
		return "", err
	}
	out, err := SimulateSeverity(rgb, deficiency, severity)
	if err != nil {
		return "", err
	}
	return Hex(out), nil
...
func SimulateSeverity(rgb RGB, deficiency Deficiency, severity float64) (RGB, error) {
	if !IsMachadoDeficiency(deficiency) {
		return RGB{}, ErrInvalidType
	}
	if math.IsNaN(severity) || math.IsInf(severity, 0) || severity < 0 || severity > 1 {
		return RGB{}, ErrInvalidSeverity
	}
	return simulateMachado(rgb, deficiency, severity), nil
}

func simulateMachado(rgb RGB, deficiency Deficiency, severity float64) RGB {
	linear := RGB{srgbToLinear(rgb.R), srgbToLinear(rgb.G), srgbToLinear(rgb.B)}
	matrix := interpolatedMachadoMatrix(deficiency, severity)
...
func interpolatedMachadoMatrix(deficiency Deficiency, severity float64) [3][3]float64 {
	series := machadoSeries(deficiency)
	scaled := severity * 10
	nearest := math.Round(scaled)
	if math.Abs(scaled-nearest) < 1e-12 {
		idx := int(nearest)
		if idx < 0 {
			idx = 0
		}
	...
	for row := 0; row < 3; row++ {
		for col := 0; col < 3; col++ {
			out[row][col] = series[lo][row][col]*(1-t) + series[hi][row][col]*t
		}
	}
	return out
}

func machadoSeries(deficiency Deficiency) *[11][3][3]float64 {
	switch deficiency {
	case Protanopia, Protanomaly:
		return &protanMatrices
	case Deuteranopia, Deuteranomaly:
		return &deutanMatrices
	case Tritanopia, Tritanomaly:
		return &tritanMatrices
	default:
		return &protanMatrices
	}
}
